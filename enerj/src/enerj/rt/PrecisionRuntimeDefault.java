...
class PrecisionRuntimeDefault implements PrecisionRuntime {
	...
	private Map<String, Long> preciseFootprint = new HashMap<String, Long>();
        ...
        protected Map<String, Double> errs = new HashMap<String, Double>(); // H3 Added
        protected Map<String, Double> abs_errs = new HashMap<String, Double>(); // H3 Added
	...
	*/
	...
	private synchronized void dumpCounts() {
	    ...
	    try {
            ...
            // H3 Added -- count of the added errors
    	    Set<String> totalErrs = new HashSet<String>();
    	    totalErrs.addAll(errs.keySet());
    	    totalErrs.addAll(abs_errs.keySet());
    	    stringer.key("totalErrors");
    	    stringer.object();
    	    for (String te : totalErrs) {
                stringer.key(te);
    	        stringer.array();
                System.out.print(te);
                System.out.print(" ");
    	        if (errs.containsKey(te)) {
                    double errAmt = errs.get(te);
                    System.out.print(errAmt);
    	            stringer.value(errAmt);

                } else if (abs_errs.containsKey(te)) {
                    double abs_errAmt = abs_errs.get(te);
                    System.out.print(abs_errAmt);
    	            stringer.value(abs_errAmt);
                }
                System.out.print("\n");
    	        stringer.endArray();
    	    }
